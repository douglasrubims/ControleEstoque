...
class ProdutoController extends Controller
{
	...
	public function adiciona(ProdutosRequest $request, ImageRepository $repo) {
		...
		if($request->hasFile('primaryImage')) $request['path_image'] = $repo->saveImage($request->primaryImage, 'produtos', 250);
	...
	public function mostra($id) {
		...
		$categorias = Categoria::all();
		if(empty($produto)) return "Essa produto não existe";
	...
	public function maiorProduto() {
		return $produto = DB::select(DB::raw("SELECT MAX(CAST(codigo_produto AS unsigned integer)) + 1 as codigo_produto FROM produtos;"));
	}
}